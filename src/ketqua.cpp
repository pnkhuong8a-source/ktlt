...
docFileKetQua()
3. Parsing dữ liệu
atoi: Chuyển đổi chuỗi ký tự thành int 
atof: Chuyển đổi chuỗi ký tự thành long
atol: huyển đổi chuỗi ký tự thành double
tachSoNguyen
4. Time Programming
time()
time_t
...
=> Dùng cho:
        trộn câu hỏi
        trộn đáp án
6. Event-driven Keyboard Input
kbhit()
getch()
=> Cho phép nhập đáp án ngay khi bấm phím.
7. Thuật toán sắp xếp nổi bọt O(n²)
sapXepKetQuaTheoThoiGian()
8. Chấm điểm tự động
if (userAnswer == correctAnswer)
9. Hệ thống thi có giới hạn thời gian thực
deadline
remain
Chương trình liên tục kiểm tra time(NULL) để tự động kết thúc khi hết giờ.
10. Theo dõi lịch sử thi
Mỗi lần thi đều lưu:
    Người thi
    Đề thi
    Câu hỏi
    ...
    Điểm
    Thời gian
=> giáo viên có thể xem lại toàn bộ lịch sử làm bài của học sinh.
*/
#include <iostream>
#include <cstring>
#include <cstdio>
